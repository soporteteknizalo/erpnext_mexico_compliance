        ...
        for account in self.parent_doc.tax_accounts:
            ...
            if tax_rate < 0:
                withholding = cfdi40.Retencion(
                    ...
                    #tasa_o_cuota=Decimal(format(Decimal(tax_rate * -1),'.6f')),
                    tasa_o_cuota=Decimal(tax_rate * -1).quantize(Decimal("0.000001")),
                )
                ...
                transferred = cfdi40.Traslado(
                    impuesto=tax_type,
                    tipo_factor=catalogos.TipoFactor.TASA,
                    tasa_o_cuota=Decimal(tax_rate).quantize(Decimal("0.000001")),
                )
