...
from ..factureHoy import factureHoy  
...
class SalesInvoice(CommonController, sales_invoice.SalesInvoice):
    ...
    if TYPE_CHECKING:
        ...
        naming_series: DF.Literal["ACC-SINV-.YYYY.-", "ACC-SINV-RET-.YYYY.-","ACC-SINV-CP.YYYY.-","ACC-SINV-TRAS.YYYY.-"]
    ...
    def attach_pdf(self) -> Document:
        ...
        #FActurehoye
        #cfdi = cfdi40.CFDI.from_string(self.mx_stamped_xml)
        #facturehoy
    ...
    def stamp_cfdi(self, certificate: str):
        ...
        '''Facture_hoy
        params = {'Contrato': '194876591', 'UserID':  "EWE1709045U0.Test", 'UserPass': "Prueba$1"}
        options = {'generarCBB': True, 'generarPDF': True, 'generarTXT': False}
        self._url_timbrado = "http://pruebasclientes.facturehoy.com/CFDI40/WsEmisionTimbrado40?wsdl"
        client = factureHoy.Cliente(self._url_timbrado, params)
        '''
        try:
            data, message = ws.stamp(cfdi)
            
            '''facturehoy
            data, message = client.timbrar(cfdi, options)
